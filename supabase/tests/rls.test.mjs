...
const newProg = await expect("admin creates a programme", "ok", as(adm, `insert into programmes (slug, title, duration_weeks, price_cents, cert_code) values ('sec','Security',6,67000,'SEC') returning id`));
await expect("instructor creates a programme", "deny", as(ins, `insert into programmes (slug, title, duration_weeks, price_cents, cert_code) values ('x','X',6,1,'XX')`));
await expect("student creates a programme", "deny", as(st, `insert into programmes (slug, title, duration_weeks, price_cents, cert_code) values ('y','Y',6,1,'YY')`));
await expect("duplicate certificate code", "deny", as(adm, `insert into programmes (slug, title, duration_weeks, price_cents, cert_code) values ('sec2','Security 2',6,1,'SEC')`));
await expect("admin adds weekly module", "ok", as(adm, `insert into programme_modules (programme_id, week, position, title) values ('${newProg.rows[0].id}',1,1,'Week 1') returning id`));
await expect("instructor adds module", "deny", as(ins, `insert into programme_modules (programme_id, week, position, title) values ('${newProg.rows[0].id}',2,1,'Week 2')`));
const newCohort = await expect("admin creates a cohort", "ok", as(adm, `insert into cohorts (programme_id, code, name, starts_on, ends_on) values ('${newProg.rows[0].id}','#03','Security','2026-11-02','2026-12-13') returning id`));
await expect("instructor creates a cohort", "deny", as(ins, `insert into cohorts (programme_id, code, name, starts_on, ends_on) values ('${PROG}','#09','X','2026-11-02','2026-12-13')`));
await expect("admin assigns the instructor", "ok", as(adm, `insert into cohort_members values ('${newCohort.rows[0].id}','${ins2}','instructor') returning user_id`));
await expect("instructor adds self to a cohort", "deny", as(ins, `insert into cohort_members values ('${newCohort.rows[0].id}','${ins}','instructor')`));
await expect("student enrols self for free", "deny", as(out, `insert into cohort_members values ('${C1}','${out}','student')`));
await expect("admin creates cohort space", "ok", as(adm, `insert into spaces (slug, name, "group", cohort_id, read_only) values ('cohort-03-general','General','Cohort #03','${newCohort.rows[0].id}',false) returning id`));
await expect("assigned instructor sees the new cohort", "ok", as(ins2, `select id from cohorts where id='${newCohort.rows[0].id}'`));
console.log("-- 0008 office hours");
const allDay = [0, 1, 2, 3, 4, 5, 6].map((d) => `('${C1}',${d},'00:00','23:59')`).join(",");
await expect("student sets office hours", "deny", as(st, `insert into office_hours values ('${C1}',0,'08:00','17:00')`));
await expect("other cohort's instructor sets hours", "deny", as(ins2, `insert into office_hours values ('${C1}',0,'08:00','17:00')`));
await expect("end before start rejected", "deny", as(ins, `insert into office_hours values ('${C1}',0,'17:00','08:00')`));
await expect("cohort instructor sets hours", "ok", as(ins, `insert into office_hours values ${allDay} returning weekday`));
await expect("student reads the hours", "ok", as(st, `select * from office_hours`));
await expect("outsider can't see the hours", "none", as(out, `select * from office_hours`));
await expect("office is open (all-day hours)", "ok", as(st, `select 1 where office_open('${C1}')`));
const th = await expect("student opens a thread while open", "ok", as(st, `insert into office_threads (cohort_id, student_id) values ('${C1}','${st}') returning id`));
const T = th.rows[0].id;
await expect("student opens a thread for someone else", "deny", as(st, `insert into office_threads (cohort_id, student_id) values ('${C1}','${st2}')`));
await expect("outsider opens a thread", "deny", as(out, `insert into office_threads (cohort_id, student_id) values ('${C1}','${out}')`));
await expect("student writes while open", "ok", as(st, `insert into office_messages (thread_id, author_id, body) values ('${T}','${st}','Stuck on NSGs') returning id`));
await expect("student writes as the instructor", "deny", as(st, `insert into office_messages (thread_id, author_id, body) values ('${T}','${ins}','fake reply')`));
await expect("classmate can't read the thread", "none", as(st2, `select * from office_messages`));
await expect("classmate can't write in the thread", "deny", as(st2, `insert into office_messages (thread_id, author_id, body) values ('${T}','${st2}','hi')`));
await expect("instructor reads the thread", "ok", as(ins, `select * from office_messages where thread_id='${T}'`));
await expect("other cohort's instructor can't read it", "none", as(ins2, `select * from office_messages`));
await as(ins, `delete from office_hours where cohort_id='${C1}'`);
await expect("office closed after hours removed", "none", as(st, `select 1 where office_open('${C1}')`));
await expect("student can't write while closed", "deny", as(st, `insert into office_messages (thread_id, author_id, body) values ('${T}','${st}','after hours')`));
await expect("instructor replies while closed", "ok", as(ins, `insert into office_messages (thread_id, author_id, body) values ('${T}','${ins}','Check the NSG priority') returning id`));
await expect("student marks the reply read", "ok", as(st, `update office_threads set student_read_at=now() where id='${T}' returning id`));
console.log("-- 0009 office hours hardening (code review)");
await as(ins, `insert into office_hours values ${allDay}`);
await expect("student moves thread to another cohort", "deny", as(st, `update office_threads set cohort_id='${C2}' where id='${T}'`));
await expect("student fakes the instructor's read time", "deny", as(st, `update office_threads set instructor_read_at=now() + interval '1 year' where id='${T}'`));
await expect("student edits last_message_at", "deny", as(st, `update office_threads set last_message_at=now() + interval '1 year' where id='${T}'`));
await expect("instructor re-points thread at another student", "deny", as(ins, `update office_threads set student_id='${st2}' where id='${T}'`));
await expect("instructor sets the student's read time", "deny", as(ins, `update office_threads set student_read_at=now() where id='${T}'`));
await expect("instructor marks thread read", "ok", as(ins, `update office_threads set instructor_read_at=now() where id='${T}' returning id`));
await expect("student marks own side read", "ok", as(st, `update office_threads set student_read_at=now() where id='${T}' returning id`));
const before = (await db.query(`select last_message_at from office_threads where id='${T}'`)).rows[0].last_message_at;
await new Promise((r) => setTimeout(r, 20));
await expect("backdated message gets server time", "ok", as(st, `insert into office_messages (thread_id, author_id, body, created_at) values ('${T}','${st}','again', now() - interval '30 days') returning (created_at > now() - interval '1 minute') as server_time`));
const after = (await db.query(`select last_message_at from office_threads where id='${T}'`)).rows[0].last_message_at;
await expect("new message bumps last_message_at", "ok", Promise.resolve({ rows: after > before ? [{ bumped: true }] : [] }));
await db.exec(`delete from office_threads where student_id='${st2}' and cohort_id='${C1}'`);
const t2 = await expect("new thread can't arrive pre-read by staff", "ok", as(st2, `insert into office_threads (cohort_id, student_id, instructor_read_at) values ('${C1}','${st2}', now() + interval '1 year') returning id, (instructor_read_at is null) as unread`));
const T2 = t2.rows[0].id;
await expect("student writes in own thread while enrolled", "ok", as(st2, `insert into office_messages (thread_id, author_id, body) values ('${T2}','${st2}','hello') returning id`));
await db.exec(`delete from cohort_members where cohort_id='${C1}' and user_id='${st2}'`);
await expect("removed student can't write in old thread", "deny", as(st2, `insert into office_messages (thread_id, author_id, body) values ('${T2}','${st2}','still here?')`));
await expect("app timezone setting is used", "ok", as(st, `select 1 where academy_tz() = 'Asia/Dubai'`));
console.log(fail ? `\n${fail} FAILED` : "\nALL PASSED");
