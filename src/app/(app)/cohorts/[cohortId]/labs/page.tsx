...
const STATUS: Record<LabStatus, { label: Key; tone: PillTone }> = {
  not_started: { label: "labs.statusNotStarted", tone: "quiet" },
  in_progress: { label: "labs.statusInProgress", tone: "neutral" },
  submitted: { label: "labs.statusSubmitted", tone: "neutral" },
  passed: { label: "labs.statusPassed", tone: "good" },
};
...
export default async function Labs({ params }: PageProps<"/cohorts/[cohortId]/labs">) {
  ...
  const card = (x: (typeof labs)[number]) => (
    <LabCard key={x.lab.id} lab={x.lab} status={x.status} student={role === "student"} now={now} t={t} />
  );

  return (
    <div className="space-y-5">
      {open.length === 0 && <Empty>{t("labs.allSubmitted")}</Empty>}
      {open.map(card)}
      <DoneGroup label={t("labs.completed")} count={done.length}>
        {done.map(card)}
      </DoneGroup>
      <OpenOnHash />
    </div>
...
function LabCard({ lab, status, student, now, t }: { lab: Lab; status: LabStatus; student: boolean; now: Date; t: T }) {
  ...
  return (
    <article key={lab.id} id={lab.id} className="scroll-mt-24 rounded-md border border-line bg-surface p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Label>{t("labs.labNumber", { number: String(lab.number).padStart(2, "0") })}</Label>
          <h2 className="mt-1 text-xl font-semibold tracking-tight">{lab.title}</h2>
        </div>
        {student && (
          <Pill tone={overdue ? "bad" : STATUS[status].tone}>
            {overdue ? t("labs.overdue") : t(STATUS[status].label)}
          </Pill>
        )}
      </div>
      ...
      <dl className="mt-4 flex flex-wrap gap-x-8 gap-y-2 font-mono text-xs">
        <div className="flex items-center gap-2">
          <dt className="font-sans text-muted">{t("labs.difficulty")}</dt>
          <dd className="flex gap-0.5" aria-label={t("labs.difficultyOf", { level: lab.difficulty })}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <dd>{t("labs.estMinutes", { count: lab.estMinutes })}</dd>
        </div>
        <div className="flex items-center gap-2">
          <dt className="font-sans text-muted">{t("labs.due")}</dt>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-line pt-5">
          {status === "not_started" ? (
            <form action={updateLab.bind(null, lab.id, "in_progress")}>
              <SubmitButton pendingLabel={t("labs.starting")}>
                <Play size={14} /> {t("labs.start")}
              </SubmitButton>
            </form>
          ) : (
            <form action={updateLab.bind(null, lab.id, "submitted")}>
              <SubmitButton variant="accent" pendingLabel={t("labs.submitting")}>
                <Send size={14} className="rtl:-scale-x-100" /> {t("labs.submit")}
              </SubmitButton>
            </form>
          )}
          <p className="text-xs text-muted">
            {status === "not_started" ? t("labs.sandboxNote") : t("labs.submitNote")}
          </p>
        </div>
      )}
    </article>
