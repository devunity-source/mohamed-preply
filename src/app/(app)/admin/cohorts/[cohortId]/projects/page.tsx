  ...
  return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.6fr_1fr] [&>*]:min-w-0">
      <div className="space-y-5">
        {projects.length === 0 && <Empty>No teams yet. Create the first one.</Empty>}
        {projects.map((view) => {
          const p = zonedParts(view.project.presentsAt ?? now);
          return (
            <ProjectCard key={view.project.id} view={view} canEdit now={now}>
              <details className="mt-5 border-t border-line pt-4">
                <summary className="cursor-pointer text-sm font-medium">Manage team</summary>
                <div className="mt-4 space-y-4">
                  ...
                  <ActionForm action={updateProject} submitLabel="Save team">
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <label className="block text-sm">
                        <span className="mb-1 block font-medium">Presentation date</span>
                        <input
                          name="date"
                          type="date"
                      ...
                      <label className="block text-sm">
                        <span className="mb-1 block font-medium">Add student</span>
                        <select name="addMember" defaultValue="" className={field} disabled={free.length === 0}>
                          <option value="">{free.length ? "Choose…" : "Everyone's on a team"}</option>
                          {free.map((s) => (
      ...
      <Card title="New team" className="self-start">
        {free.length === 0 ? (
          <p className="text-sm text-muted">Every student is on a team. Remove someone from a team to regroup.</p>
        ) : (
          <ActionForm action={createProject} submitLabel="Create team" resetOnSuccess>
            <input type="hidden" name="cohortId" value={cohortId} />
            <label className="block text-sm">
              <span className="mb-1 block font-medium">Team name</span>
              <input name="teamName" required maxLength={80} placeholder="Team Echo" className={field} />
            </label>
            <label className="block text-sm">
              <span className="mb-1 block font-medium">Project title</span>
              <input name="title" maxLength={140} placeholder="Capstone project" className={field} />
            </label>
            <fieldset className="text-sm">
              <legend className="mb-1 font-medium">Members</legend>
