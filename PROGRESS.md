...
| Design (whole site and app) | Taken from the owner's own starting design (25 Sep 2026): dark navy `#0F172A`, slate cards with white-at-10% borders, violet `#7C3AED` to cyan `#22D3EE` gradient for highlighted words (filled buttons end on a deeper `#0E7490` so white text stays at least 5.4:1), pink `#F472B6` as a third accent. Sora for headings (and all marketing text), Inter for app text, Noto Kufi Arabic for Arabic. One theme, no light mode. Tokens in `globals.css`. | Owner asked for the app to match the design they started with. Replaces the earlier paper/orange app look and the Dusk/Archivo marketing palette. |
