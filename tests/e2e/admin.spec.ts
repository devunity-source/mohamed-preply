...
test("issue, verify and revoke a certificate", async ({ page, anon }) => {
  await page.goto("/admin/cohorts/c_devops_00/certificates");
  ...
  const id = (await main(page).innerText()).match(/ACM-DEV-\d{4}-00003/)![0];
  ...
  const row = main(page).locator("li, tr, div").filter({ hasText: id }).last();
...
test("price and publishing changes show on the landing page", async ({ page }) => {
  await page.goto("/admin/programmes/p_ai");
...
test("create a programme: duplicate certificate codes are refused; it starts as a draft", async ({ page }) => {
  ...
  await f.getByRole("button", { name: "Create programme" }).click();
  await expect(page).toHaveURL(/\/admin\/programmes\/p_\w+\?created=1$/);
...
test("create a cohort: past dates are refused; the instructor is told and gets its spaces", async ({ page, as }) => {
  await page.goto("/admin/cohorts/new?programme=p_devops");
  const f = main(page).locator("form");
  await f.locator("select[name=instructorId]").selectOption("u_samira");
  ...
  await f.getByRole("button", { name: "Create cohort" }).click();
  await expect(page).toHaveURL(/\/admin\/cohorts\/c_\w+$/);
...
test("add an existing student and a new one; remove a student", async ({ page, as }) => {
  ...
  expect((await ines.goto("/cohorts/c_ai_02"))?.status()).toBe(404);
});
