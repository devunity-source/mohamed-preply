...
import { academyWeekday, main, resetData, signIn, expect, test } from "./helpers";
...
async function setHours(page: Page, days: number[], start = "00:00", end = "23:59") {
  await page.goto("/admin/cohorts/c_devops_01/office-hours");
...
async function sendAsStudent(student: Page, text: string) {
  await student.goto("/cohorts/c_devops_01/office-hours");
...
test("outside office hours the message box is greyed out, and the server refuses a forced send", async ({
  ...
  await ahmed.goto("/cohorts/c_devops_01/office-hours");
...
test("during office hours: send, get notified, reply, see the reply", async ({ page, as }) => {
  ...
  await page.goto("/admin/cohorts/c_devops_01/office-hours?student=u_ahmed");
...
test("opening a conversation clears the unread count without reloading", async ({ page, as }) => {
  ...
  await page.goto("/admin/cohorts/c_devops_01");
...
test("conversations are private, and removed students lose office hours", async ({ page, as }) => {
  ...
  await maria.goto("/cohorts/c_devops_01/office-hours");
  ...
  expect((await samira.goto("/admin/cohorts/c_devops_01/office-hours"))?.status()).toBe(404);
  ...
  expect((await ahmed.goto("/cohorts/c_devops_01/office-hours"))?.status()).toBe(404);
});
